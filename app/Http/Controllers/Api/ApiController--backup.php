...
class ApiController extends BaseController
{
    ...
    public function homepage(Request $request){
        if ($request->header('Authorization') == ApiController::AUTHORIZATION_TOKEN) {
            ...
            $categories = DB::table('categories')->where('is_audio', $request->book_type)->get();
            return response()->json([
                'success' => true,
                'data' => HomepageResource::collection($categories)
            ]);
    ...
    public function bestSellingBooks(Request $request){
        if ($request->header('Authorization') == ApiController::AUTHORIZATION_TOKEN) {

            $lastWeekStartDate = date("Y-m-d", strtotime("-7 days", strtotime(date("Y-m-d"))))." 00:00:00";
            $queryEndDate = date("Y-m-d")." 23:59:59";

            $lastWeekBestSellingBooks = DB::table('order_details')
                    ->leftJoin('orders', 'order_details.order_id', 'orders.id')
                    ->join('products', 'order_details.product_id', '=', 'products.id')
                    ->leftJoin('categories', 'products.category_id', '=', 'categories.id')
                    ->leftJoin('units', 'products.unit_id', '=', 'units.id')
                    ->leftJoin('brands', 'products.brand_id', '=', 'brands.id')
                    ->selectRaw('products.*, categories.name as category_name, units.name as unit_name, brands.name as brand_name, SUM(order_details.qty) as total_qty')
                    ->whereBetween('order_details.created_at', [$lastWeekStartDate, $queryEndDate])
                    ->where('orders.payment_status', 1)
                    ->where('products.is_audio', $request->book_type) //Audio
            ...
            return response()->json([
                'success' => true,
                'data' => ProductResource::collection($lastWeekBestSellingBooks)
            ]);
