    ...
    public function filterSearchResults(Request $request){
        if ($request->header('Authorization') == FilterController::AUTHORIZATION_TOKEN) {
            ...
            $data = DB::table('products')
                        ...
                        ->select('products.*', 'categories.name as category_name', 'subcategories.name as subcategory_name', 'units.name as unit_name', 'brands.name as brand_name', 'product_models.name as model_name', 'product_warrenties.name as product_warrenty')
        ...
        if ($request->header('Authorization') == FilterController::AUTHORIZATION_TOKEN) {
            ...
            if($categoryId || $subcategoryId || $childcategoryId || $flagId || $brandId || $sortBy || $priceRangeMin || $priceRangeMax || $warrentyId || $searchKeyword){ //$storageId || $deviceConditionId || $simTypeId || $regionId ||
                $data = DB::table('products')
                            ...
                            ->leftJoin('product_warrenties', 'products.warrenty_id', '=', 'product_warrenties.id')
                            ->leftJoin('product_variants', 'products.id', '=', 'product_variants.product_id')
                            ->select('products.*', 'categories.name as category_name', 'subcategories.name as subcategory_name', 'child_categories.name as childcategory_name', 'units.name as unit_name', 'brands.name as brand_name', 'product_models.name as model_name', 'product_warrenties.name as product_warrenty')
                            ->where('products.status', 1)
